...
usage() {
  echo "Usage: NDK_ROOT=<path to ndk root> $(basename "$0") [-a:s:t:Tx:X]"
  echo "-a [Architecture] Architecture of target android [default=armeabi-v7a] \
(supported architecture list: arm64-v8a armeabi-v7a)"
...
while getopts "a:Es:t:Tx:" opt_name; do
  case "$opt_name" in
...
if [[ "${ONLY_MAKE_TENSORFLOW}" != "true" ]]; then
  ...
  tensorflow/contrib/makefile/compile_android_protobuf.sh -c -a "${ARCHITECTURE}"
else
...
SUB_MAKEFILES="${SUB_MAKEFILES}" ${EXTRA_MAKE_ARGS[@]} ANDROID_ARCH="${ARCHITECTURE}"
else
